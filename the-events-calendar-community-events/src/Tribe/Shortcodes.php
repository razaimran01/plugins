...
class Tribe__Events__Community__Shortcodes extends Tribe__Events__Community__Shortcode__Abstract {
	...
	public function enqueue_assets() {
		tribe( 'community.main' )->maybeLoadAssets( true );
		tribe( 'community.main' )->enqueue_assets();
	...
	public function do_shortcode( $attributes = [], $tag = 'tribe_community_events' ) {
		...
		tribe( 'community.main' )->default_template_compatibility();
		...
		switch ( $tribe_attributes['view'] ) {
			case 'submission_form':
				$view = tribe( 'community.main' )->doEventForm( null, true );
				break;
			case 'my_events':
				$view = tribe( 'community.main' )->doMyEvents( $page_number, false, true );
				break;
			case 'edit_event':
				if ( $tribe_id === false ) {
					...
					$view = tribe( 'community.main' )->doEventForm( $tribe_id, true );
				}
				break;
			case 'edit_venue':
				if ( $tribe_id === false ) {
					...
					$view = tribe( 'community.main' )->doVenueForm( $tribe_id );
				}
				break;
			case 'edit_organizer':
				if ( $tribe_id === false ) {
					...
					$view = tribe( 'community.main' )->doOrganizerForm( $tribe_id );
				}
				break;
			default:
